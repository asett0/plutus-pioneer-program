...
mkValidator d _ s =
  signedByBeneficiary1 && (to (deadline d) `contains` range)
    || signedByBeneficiary2 && (from (1 + deadline d) `contains` range)
  where
    info :: TxInfo
    info = scriptContextTxInfo s

    signedByBeneficiary1 :: Bool
    signedByBeneficiary1 = txSignedBy info $ beneficiary1 d

    signedByBeneficiary2 :: Bool
    signedByBeneficiary2 = txSignedBy info $ beneficiary2 d
